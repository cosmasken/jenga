...
import { useReadContract, useWriteContract, useWaitForTransactionReceipt, useWatchContractEvent, WriteContractReturnType } from 'wagmi';
import { TESTNET_CONFIG } from '../config';
import SACCOABI from '../abi/SACCO.json';

export const SACCO_CONTRACT = {
  address: TESTNET_CONFIG.smartContracts.sacco as Address, // Placeholder: Replace with actual deployed SACCO contract address
  abi: SACCOABI as Abi,
} as const;
...
export function useSacco() {
  ...
  const registerMember = async (memberAddress: Address) => {
    const { hash }: WriteContractReturnType = await writeContractAsync({
      abi: SACCO_CONTRACT.abi,
      address: SACCO_CONTRACT.address,
